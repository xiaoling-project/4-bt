...
// 创建总大小为16K*1024即16MB的缓冲区,btcache_len为1024,每个Btcache的大小为16kb
int create_btcache()
{
	int     i;
	Btcache *node, *last;
	...
	*/
	...
	最后一个piece的数据将存储于此处
	*/
	int count = file_length % piece_length / (16*1024); //判断最后一个piece有多少个slice
	if(file_length % piece_length % (16*1024) != 0)  count++;最后一个slice没有16kb,则slice数目加1
	last_piece_count = count; //存储最后一个piece有多少个slice

	last_slice_len = file_length % piece_length % (16*1024); //最后一个slice有多大
	if(last_slice_len == 0)  last_slice_len = 16*1024;
	
	last_piece_index = pieces_length / 20 -1; //最后一个piece的索引,每个piece的hash长度为20
...
int create_files()
{
	...
	if( is_multi_files() == 0 ) {  // 待下载的为单文件
		...
		*fds = open(file_name,O_RDWR|O_CREAT,0777);//通过文件名来创建相对应的文件
...
//这个是用于write_piece_to_harddisk()函数的
int write_btcache_node_to_harddisk(Btcache *node)
{
	...
	while(p != NULL) {
		if((line_position < p->length) && (line_position+node->length < p->length)) {
			...
			i++;
			p = p->next;
		}//每次都要循环输入,这样的速度是不是变慢了,文件数目不会太多,速度差距不大
	}
	return 0;
}
...
//读取相应的slice,这个是用于将一个piece写入缓冲区中
...
int delete_request_end_mode(int index)
{
	...
	//通过循环Peer链表,对其中的Request_piece链表的遍历,来删除所对应的piece请求
	while(p != NULL) {
...
//检查一个piece的数据是否正确,若正确存入文件，通过sha1.c的函数来对生成相对应的hash值,然后与解析文件所得的hash值进行比较
int write_piece_to_harddisk(int sequnce,Peer *peer)
{
	...
	int            slice_count = piece_length / (16*1024); //slice的数目
	int            index, index_copy;

	if(peer==NULL) return -1;

	int i = 0;
	//sequence存储了要放入文件的piece的第一个slice位于缓冲区的第几个节点
	while(i < sequnce)  { node_ptr = node_ptr->next; i++; }
	
	p = node_ptr;  // p指向待存储的piece的第一个slice所在的btcache结点
	...
	//找到pieces存放该pieces的hash值的位置
	index = p->index * 20; //每个piece的hash值的长度为20
	index_copy = p->index;  //存放待放入硬盘的piece的索引
	for(i = 0; i < 20; i++)  piece_hash2[i] = pieces[index+i];

	int ret = memcmp(piece_hash1,piece_hash2,20); //将两个hash值进行比较
	if(ret != 0)  { printf("piece hash is wrong\n"); return -1; }
	
	
	//上面检验成功之后,便将piece存储到文件中
	node_ptr = p;
...
int write_btcache_to_harddisk(Peer *peer)
{
	...
	while(p != NULL) {
		...
		//每经过slice_count个节点都要将full_count置为零,以便判断是否有完整的piece
...
int is_a_complete_piece(int index, int *sequnce)
{
	...
	while(p != NULL) {
		...
		//找到放该索引为index的piece的第一个Btcache节点
		...
		//sequnce存储了前面有多少了slice,是用来定位要将第几个缓冲区存储到硬盘中去
		*sequnce = count;
		num = slice_count;
	
		//计算该piece总共下载了多少slice
		while(num>0 && p!=NULL) {
						   ...
						   unsigned char *buff,int len,Peer *peer)
{
	...
	Btcache *p = btcache_head, *q = NULL;  //p指向了缓冲区的头结点,q指向每个piece第一个slice
	
	if(p == NULL)  return -1;
	
	
	//index指明pieces索引,index最大为pieces_length/20 - 1
	//一个slice的大小为16kb,如果存储的位置超过存储最后一个slice的起始位置,则发生错误
	
	if(index>=pieces_length/20 || begin>piece_length-16*1024)  return -1;
	if(buff==NULL || peer==NULL)  return -1;

	//判断是不是最后一个piece,是的话将其写入到最后一个piece的缓冲区中
	...
	slice_count = piece_length / (16*1024); 
	while(p != NULL) {
		if(count%slice_count == 0)  q = p; //这里的q总是指向缓冲区中,piece的第一个slice的btcache节点的指针
		if(p->index==index && p->in_use==1)  break; //该piece已经下载了一部分内容了

		count++; //计数当前指向第几个缓冲区节点
		p = p->next;
	}
	
	// p非空说明当前slice所在的piece的有些数据已经下载
	if(p != NULL) {
		count = begin / (16*1024);  // count存放值为当前piece的第几个slice
		p = q;
		...
		memcpy(p->buff,buff,len);//将数据从peer缓冲区拷贝到缓冲模块缓冲区中
		printf("+++++ write a slice to btcache index:%-6d begin:%-6x +++++\n",
			   index,begin);
		
		// 如果是刚刚开始下载(下载到的piece不足10个),则立即写入文件中
		//这样其他的peer便可马上知道该piece在我这里已经下载成功了
	...
	while(i > 0) {
		...
		while(p != NULL) {
			...
			if(unuse_count == slice_count)  break;  //找到一个空闲的,可以存放piece的内存空间
			
			count++;//用于取余运算,指明当前指向缓冲区的第几个节点
			p = p->next;
		}
...
int read_slice_for_send(int index,int begin,int length,Peer *peer)
{
	...
	while(i > 0) {
		...
		//这个跟下载一个新的piece时寻找缓冲区来存放的过程是一样的
		while(p != NULL) {
			if(count%slice_count == 0)  { unuse_count = 0; q = p; }
			if(p->in_use == 0) unuse_count++;
